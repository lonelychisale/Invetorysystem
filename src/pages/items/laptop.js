...
const handleSearchSubmit = async (e) => {
  ...
  try {
    const response = await axios.get('http://localhost:8000/api/laptopsearch', {
      params: searchInput,
    });
...
const [loading, setLoading] = useState(true);
  ...
  const itemsRetrieval = async () => {
    try {
      const res = await axios.get('http://localhost:8000/api/getlaptops');
      console.log(res);

      if (res.data.status === 200) {
        const itemsData = res.data.laptops;
  ...
  if (loading) {
    ...
    itemstable = (
      <tr>
        <td colSpan='10'>There are no computers in the database</td>
      </tr>
    );
  } else {
