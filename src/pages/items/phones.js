...
const handleSearchSubmit = async (e) => {
  ...
  try {
    const response = await axios.get('http://localhost:8000/api/phonesearch', {
      params: searchInput,
    });
...
const [loading, setLoading] = useState(true);
  ...
  const itemsRetrieval = async () => {
    try {
      const res = await axios.get('http://localhost:8000/api/getphones');
      console.log(res);

      const itemsData = res.data.phones;
  ...
  if (loading) {
    ...
    itemstable = (
      <tr>
        <td colSpan='10'>There are no phones in the database</td>
      </tr>
    );
  } else {
