...
function Viewstationary(){
  ...
  const handleSearchSubmit = async (e) => {
    ...
    try {
      const response = await axios.get('http://localhost:8000/api/stationarysearch', {
        params: searchInput,
      });
...
const [loading, setLoading] = useState(true);
    ...
    const itemsretrieval = async () =>{
        const res =  await axios.get('http://localhost:8000/api/getstationary')
        console.log(res)

        if(res.data.status === 200){
          const itemsData = res.data.stationary;
  ...
  if (loading) {
    ...
    itemstable = (
      <tr>
        <td colSpan='10'>There are no stationaries in the database</td>
      </tr>
    );
  } else {
